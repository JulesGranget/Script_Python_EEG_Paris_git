...
def get_pli_ispc_fc_dfc_trial(sujet, cond, odor_i, band_prep, band, freq):
    ...
    convolutions = np.memmap(f'{sujet}_{cond}_{band_prep}_{band}_{odor_i}_fc_convolutions.dat', dtype=np.complex128, mode='w+', shape=(len(chan_list_eeg), nfrex, data_length))
    ...
    os.chdir(path_memmap)
    res_fc_phase = np.memmap(f'{sujet}_{cond}_{band_prep}_{band}_{odor_i}_fc_phase.dat', dtype=np.float32, mode='w+', shape=(2, len(pairs_to_compute), len(phase_list), nfrex))
...
def get_wpli_ispc_fc_dfc(sujet, cond, band_prep, band, freq):
    ...
    for odor_i in odor_list:
        ...
        dict_xr = {'mat_type' : ['ispc', 'wpli'], 'pairs' : pairs_to_compute, 'phase' : ['whole', 'inspi', 'expi'], 'nfrex' : range(nfrex)}
