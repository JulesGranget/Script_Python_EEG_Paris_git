...
from n17_precompute_allsujet_ERP import *
...
def plot_ERP_diff(stretch=True):
    ...
    if stretch:
        ...
        cluster_stats_intra, cluster_stats_inter, cluster_stats_rep_norep = get_cluster_stats_manual_prem1d(stretch=False)
    ...
    for nchan_i, nchan in enumerate(chan_list_eeg):
        ...
        #### save
        os.chdir(os.path.join(path_results, 'allplot', 'ERP', 'summary_diff', 'repnorep'))

        if stretch:

            fig.savefig(f'stretch_{nchan}_repnorep_{group}.jpeg', dpi=150)
            fig.clf()
            plt.close('all')
            gc.collect()
            ...
            fig.savefig(f'nostretch_{nchan}_repnorep_{group}.jpeg', dpi=150)
